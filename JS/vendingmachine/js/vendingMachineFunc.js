class VendingMachineFunc { // 객제지향형으로 작성하기 위해 class 사용
  constructor() {
    // section 1 요소 선택
    const vMachine = document.querySelector('.section1');
    this.balance = vMachine.querySelector('.bg-box p'); // this로 인스턴스 생성해서 관련 기능 대량 생산 가능
  ...
  bindEvents() {
    ...
    colaBtns.forEach((colaBtn) => {
      colaBtn.addEventListener('click', (event) => {
        ...
        if (balanceVal >= targetElPrice) {
          ...
          // stagedListItem.forEach(() => { // forEach는 특정 조건이 만족될 경우 break로 정지해야 하는데 그게 불가능해서 비효율적
          // });
    ...
    this.btnGet.addEventListener('click', () => {
      ...
      this.textTotal.textContent = `총 금액 : ${new Intl.NumberFormat().format(totalPrice)} 원`;
    });
  }
}
