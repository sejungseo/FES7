...
let arr = [10, 20, 30, 40, 50]
let sum = 0;
arr.forEach((v) => { // 인자값이 하나면 괄호 생략가능 v
    sum += v
})
console.log(sum)
...
sum // 5050, 수학 수식으로 계산되는 코드는 이렇게 순회를 돌면서 풀지 않으셔도 됩니다. 이 코드는 좋은 코드는 아닙니다.
...
let sortValue = userInput.sort((a, b) => b - a) //userInput이 정렬되어버림

let userInput = [3, 76, 24]
let sortValue = [...userInput].sort((a, b) => b - a) //userInput 배열 그대로 유지
...
// 문제6. 1부터 100까지의 숫자 중 3의 배수만 더해주세요.
let sum = 0;
...
// 문제8. 010-5044-2903 번호가 있었을 때 아래와 같이 출력되게 해주세요. 뒤에 2자리를 x로 바꿔주세요.
// input = '010-5044-2903'
...
'010-4444-2903'.split('-').forEach((v, i) => { // 4444 오류발생
    console.log(v.replace(v.slice(-2), 'xx'))  //v.slice(-2) = 44 라서 replace 적용으로 앞에 나온 44를 먼저 바꿔줌
})
...
[1, 2, 3].forEach(v => {
  console.log(v)
  break // break는 error 입니다.(반복문에서 배울 예정)
})

[1, 2, 3].forEach(v => {
  console.log(v)
  continue // continue는 error 입니다.(반복문에서 배울 예정)
})
